...
def _load_windows_token() -> None:
    """Make the persisted user token available even in an already-open terminal."""
    if os.getenv("FOOTBALL_DATA_API_TOKEN") or os.name != "nt":
        return
    try:
        import winreg
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment") as key:
            value, _ = winreg.QueryValueEx(key, "FOOTBALL_DATA_API_TOKEN")
            if value:
                os.environ["FOOTBALL_DATA_API_TOKEN"] = value
